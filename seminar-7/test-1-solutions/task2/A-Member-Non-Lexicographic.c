...
void find_and_print_subsequences(colleague_t *order_array, int num_collegues,
  ...
  for (int coleg_idx = current_num_signs; coleg_idx < num_collegues;
       ++coleg_idx) {
...
int compare_collegues(const void *lhs, const void *rhs) {
  ...
  if (left_colleague->num_signs_req != right_colleague->num_signs_req)
    return left_colleague->num_signs_req - right_colleague->num_signs_req;

  // Если количество требуемых подписей одинаковое — сравниваем по исходным
  // индексам
  return left_colleague->original_idx - right_colleague->original_idx;
}
